def poor_hash(key):
    # Função de hash ruim: apenas retorna o comprimento da chave
    return len(key)


def good_hash(key):
    # Função de hash boa: usa a função hash padrão do Python
    return hash(key) % 10


# Teste com várias chaves
keys = ["Fulano", "Ciclano", "Beltrano", "Victor"]
print("Poor Hash:")
for key in keys:
    print(f"Key: {key}, Hash: {poor_hash(key)}")

print("\nGood Hash:")
for key in keys:
    print(f"Key: {key}, Hash: {good_hash(key)}")


def custom_hash(key):
    # Função de hash simples personalizada
    hash_value = 0
    for char in key:
        hash_value += ord(char)  # Soma os valores ASCII de cada caractere
...
for key in keys:
    print(f"Key: {key}, Hash: {custom_hash(key)}")
